<template>
  <div style="background:#fff ;padding:25px;position: relative">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="edit" @click="handleShop('edit')">设置关联业务员</a-button>
      <!-- <a-dropdown>
        <a-menu slot="overlay">
          <a-menu-item key="1" @click="handleShop('pass')">
            <a-icon type="check-circle" />通过
          </a-menu-item>
          <a-menu-item key="2" @click="handleShop('notpass')">
    ...
    <!-- 新增编辑模态框 -->
    <a-modal :width="editType=='edit'?'400px':editType=='detail'?'800px':'400px'" :title="modalTitle" :visible="visible" @ok="handleOk" :confirmLoading="confirmLoading" @cancel="handleCancel">
      <div class="modal-container" v-if="editType=='edit'">
        <a-form>
          <a-row :gutter="24">
            <a-col :md="24" :sm="24">
              <a-form-item label="关联业务员" :required="true">
                <a-select v-model="forms.salemanId" :disabled="editType=='detail'" placeholder="请选择业务员">
          ...
          <a-row :gutter="24">
            <a-col :md="12" :sm="24">
              <a-form-item label="店铺名称" :required="true">
                <a-input v-model="forms.shopName" :disabled="true" placeholder="店铺名称" />
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item label="联系人电话" :required="true">
                <a-input v-model="forms.phoneNumber" maxlength="11" :disabled="true" placeholder="请输入有效的手机号" />
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item label="店铺类型" :required="true">
                <a-select v-model="forms.shopType" :disabled="true" placeholder="请选择">
                  <a-select-option value="零售">零售</a-select-option>
                  <a-select-option value="餐饮">餐饮</a-select-option>
                  <a-select-option value="外卖">外卖</a-select-option>
                </a-select>
            ...
            <a-col :md="12" :sm="24">
              <a-form-item label="关联收呗店铺号">
                <a-input v-model="forms.shopNo" :disabled="true" placeholder="请输入关联收呗店铺号" />
              </a-form-item>
            </a-col>
            ...
            <a-col :md="12" :sm="24">
              <a-form-item label="关联业务经理" :required="true">
                <a-select v-model="forms.salemanId" :disabled="true" placeholder="请选择业务经理">
                  <a-select-option :value="v.id" v-for="(v,i) of bdList" :key="i" :disabled="v.state=='disabled'?true:false">{{v.salesmanName}}{{v.state=='disabled'?'（该业务员已被禁用）':''}}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            ...
            <a-col :md="12" :sm="24">
              <a-form-item label="店铺地址">
                <a-input v-model="forms.shopAddress" :disabled="true" placeholder="请输入店铺地址" />
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item label="店铺经度">
                <a-input v-model="forms.shopLongitude" :disabled="true" placeholder="请输入店铺经度" />
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item label="店铺维度">
                <a-input v-model="forms.shopLatitude" :disabled="true" placeholder="请输入店铺维度" />
              </a-form-item>
            </a-col>

            <a-col :md="12" :sm="24" v-show="editType=='detail'">
              <a-form-item label="审核">
                <a-select v-model="forms.auditState" :disabled="true" placeholder="请选择">
                  <a-select-option value="pass">通过</a-select-option>
                  <a-select-option value="notpass">不通过</a-select-option>
                  <a-select-option value="not">未审核</a-select-option>
                </a-select>
            ...
            <a-col :md="24" :sm="24">
              <a-form-item label="店铺简介">
                <quill-editor v-model="shopDescription" :options="editorOption" :disabled="true" style="height:200px" ref="myQuillEditor"></quill-editor>
              </a-form-item>
            </a-col>
...
import { getShopInfo, getShopList, enabledShop, disabledShop, modifyShop, getBdListAll } from '@/api/common'
...
export default {
  ...
  methods: {
    ...
    // 数据校验
    checkEditData() {
      if (!this.forms.salemanId) {
        this.$message.warning('请关联业务经理！')
        return false
      } else {
        return true
      }
        ...
        .then(res => {
          ...
          if (res.code == 0) {
            ...
            if (res.result.list.length > 0) {
              ...
              // this.setQrCode(res.result.list)
            } else {
              this.loadDatas = []
            }
          } else {
